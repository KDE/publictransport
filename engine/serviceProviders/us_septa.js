/** Service provider for septa.org (Southeastern Pennsylvania, USA)
* © 2010, Friedrich Pülz */
...
function parseTimetable( html ) {
	...
	var resultTableRegExp = /<div [^>]*id="HFSResult"[^>]*>[\s\S]*<table [^>]*class="resultTable"[^>]*>([\s\S]*<tr>[\s\S]*<th[\s\S]+)<\/table>/i;
    ...
    if ( (meanings = meaningsRegExp.exec(str)) == null ) {
		helper.error("Result table header not found!", str);
		return;
	}
    meanings = meanings[1];
    ...
    if ( timeCol == -1 || typeOfVehicleCol == -1 || targetCol == -1 ) {
		helper.error("Not all column meanings were found!", meanings);
		return;
	}
...
//     var targetRegExp = /<span class="bold">[^<]*?<a[^>]*?>([\s\S]*?)<\/a>/i;
    var targetRegExp = /<strong>[^<]*?<a[^>]*?>([\s\S]*?)<\/a>/i;
...
function parseStopSuggestions( html ) {
    // Find block of stops
	var stopRange = /<select [^>]*class="error"[^>]*>([\S\s]*)<\/select>/i.exec( html );
	if ( stopRange == null ) {
		helper.error("Stop range not found!", html);
		return false;
	}
	stopRange = stopRange[1];

    // Initialize regular expressions (compile them only once)
    var stopRegExp = /<option value="[^"]+?#([0-9]+)">([^<]*?)<\/option>/ig;

    // Go through all stop options
    while ( (stop = stopRegExp.exec(stopRange)) ) {
		var stopID = stop[1];
		var stopName = stop[2];
