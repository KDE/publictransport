...
<tr><td><i>RouteNews</i></td> <td>QStringList</td> <td>A list of news/comments for sub-journeys. If 'RouteStops' and 'RouteNews' are both set, the latter contains one element less (one news/comment string for each sub-journey between two stop from 'RouteStops').</td></tr>
...
<tr><td><i>RouteSubJourneys</i></td> <td>QList< QVariantMap ></td> <td>A list of data maps for all sub-journeys between two connecting stops. If 'RouteStops' and 'RouteSubJourneys' are both set, the latter contains one element less (one sub-journey between two stops from 'RouteStops'). Each map in the list contains route data for the sub journey. These TimetableInformation values can be used inside this map: RouteStops, RouteNews, RouteTimesDeparture, RouteTimesArrival, RouteTimesDepartureDelay, RouteTimesArrivalDelay, RoutePlatformsDeparture and RoutePlatformsArrival. Each list should contain the same number of elements here (no origin or target included here, only intermediate stops).</td></tr>
</table>
