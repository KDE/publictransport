...
function parseTimetable( html ) {
    ...
    while ( (departureArr = departuresRegExp.exec(str)) ) {
        ...
        var vehicle = helper.trim( typeOfVehicle[1] ).toLowerCase();
        if ( vehicle == "nbu" ) {
            departure.TypeOfVehicle = "bus";
        } else if ( vehicle == "s1" ) {
            departure.TypeOfVehicle = "s";
        } else if ( vehicle == "ntr" || vehicle == "tra" ) {
            departure.TypeOfVehicle = "tram";
        } else {
...
//         journey.ArrivalTime = arrivalTime.hour + ":" + arrivalTime.minute;
        ...
        print( "ARRIVAL DATE/TIME: " + arrivalTime.hour + ":" + arrivalTime.minute );
        ...
        for ( var n = 0; n < types.length; ++n ) {
            journey.TypesOfVehicleInJourney.push( helper.trim(types[n]).replace(/\d$/, '') );
        }
