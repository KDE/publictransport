...
function parseTimetable( html ) {
    ...
    if ( (meanings = meaningsRegExp.exec(str)) == null ) {
		println("Couldn't read column meanings! Website layout may have changed.");
		return;
	}
    meanings = meanings[1];
    ...
    if ( timeCol == -1 || typeOfVehicleCol == -1 || targetCol == -1 ) {
		println("Couldn't find time, target and type of vehicle columns! Website layout may have changed.");
		return;
	}
    ...
    while ( (departureRowArr = departuresRegExp.exec(str)) ) {
		...
		if ( (typeOfVehicle = typeOfVehicleRegExp.exec(transportLine)) == null ) {
			println("Unexpected string in type of vehicle column: \"" + transportLine + "\"! Website layout may have changed.");
