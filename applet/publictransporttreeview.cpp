...
void HeaderView::paintEvent( QPaintEvent* e )
{
	...
	svg.setEnabledBorders( Plasma::FrameSvg::TopBorder | Plasma::FrameSvg::BottomBorder
						   | Plasma::FrameSvg::LeftBorder | Plasma::FrameSvg::RightBorder );
...
TreeView::TreeView( QStyle* style ) : QTreeView()
{
	...
	m_noItemsText = i18nc("@info/plain Displayed in the view if no items are contained", "No items.");
}
						...
						const QModelIndex& index ) const
{
	...
	if (( scrollValue < verticalScrollBar()->maximum() &&
				...
				|| (scrollValue > 0 && isHeaderHidden() && topFadeRect.intersects(options.rect)) ) { // row gets faded on top or bottom
