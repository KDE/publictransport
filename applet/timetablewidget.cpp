...
PublicTransportGraphicsItem::PublicTransportGraphicsItem( QGraphicsItem* parent,
        ...
        : QGraphicsWidget(parent), m_item(0), m_parent(0), m_resizeAnimation(0), m_pixmap(0),
    ...
    if ( item->hasAlarm() ) {
        if ( item->alarmStates().testFlag(AlarmIsAutoGenerated) ) {
            removeAlarmAction = new QAction( KIcon("task-reminder"),
                    i18n("Remove &Alarm For This Journey"), &contextMenu );
...
//             } else {
                infoAction = new QAction( i18nc("@info/plain", "(has recurring/multiple alarms)"), this );
//             }
        } else {
            // The 'Remove this Alarm' menu entry can only be
            // used with autogenerated alarms
...
//             } else {
                infoAction = new QAction( i18nc("@info/plain", "(has custom/multiple alarms)"), this );
//             }
        }
        if ( infoAction ) {
...
DepartureGraphicsItem::DepartureGraphicsItem( QGraphicsItem* parent,
        ...
        : PublicTransportGraphicsItem( parent, copyStopToClipboardAction/*, toggleAlarmAction*/ ),
