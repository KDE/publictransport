...
void PublicTransport::writeSettings( const Settings& settings )
{
    ...
    if ( changed.testFlag(SettingsIO::IsChanged) ) {
        ...
        if ( changed.testFlag(SettingsIO::ChangedAlarmSettings) && m_modelJourneys ) {
            for ( int row = 0; row < m_modelJourneys->rowCount(); ++row ) {
                ...
                JourneyItem *journeyItem = static_cast<JourneyItem*>( m_modelJourneys->item(row) );
                ...
                for ( int a = 0; a < m_settings.alarmSettings.count(); ++a ) {
                    AlarmSettings alarmSettings = m_settings.alarmSettings.at( a );
                    if ( alarmSettings.affectedStops.contains(m_settings.currentStopSettingsIndex)
                         && alarmSettings.enabled && alarmSettings.filter.match(departureInfo) )
                    {
                        // TODO Could be fired already?
        ...
        QGraphicsWidget *item )
{
    ...
    if ( !departure.isNull() ) {
        alarm.filter.append( Constraint(FilterByDeparture, FilterEquals, departure) );
    }
    if ( !lineString.isEmpty() ) {
        alarm.filter.append( Constraint(FilterByTransportLine, FilterEquals, lineString) );
    }
    alarm.filter.append( Constraint(FilterByVehicleType, FilterIsOneOf,
                                    QVariantList() << vehicleType) );
    if ( !target.isEmpty() ) {
        alarm.filter.append( Constraint(FilterByTarget, FilterEquals, target) );
    }
