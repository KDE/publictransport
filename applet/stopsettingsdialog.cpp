...
StopSettingsDialog::StopSettingsDialog( const StopSettings &stopSettings,
		...
		Plasma::DataEngine *osmEngine, Plasma::DataEngine *geolocationEngine, QWidget *parent )
		: KDialog(parent), m_stopFinder(0), m_nearStopsDialog(0),
		m_modelLocations(modelLocations), m_modelServiceProviders(modelServiceProviders),
		m_modelLocationServiceProviders(0), m_publicTransportEngine(publicTransportEngine),
...
#ifdef USE_KCATEGORYVIEW
	KCategorizedSortFilterProxyModel *modelCategorized = new KCategorizedSortFilterProxyModel( this );
...
void StopSettingsDialog::setStopSettings( const StopSettings& stopSettings )
{
	...
	m_uiStopDetails.timeOfFirstDeparture->setValue( stopSettings.timeOffsetOfFirstDeparture );
	m_uiStopDetails.timeOfFirstDepartureCustom->setTime( stopSettings.timeOfFirstDepartureCustom );
...
// 	    .data( ServiceProviderDataRole ).toHash();
	...
	stopSettings.firstDepartureConfigMode = m_uiStopDetails.firstDepartureUseCurrentTime->isChecked()
        ...
        const QString& city, qreal /*latitude*/, qreal /*longitude*/, int accuracy )
{
	...
	connect( m_nearStopsDialog, SIGNAL(finished(int)), this, SLOT(nearStopsDialogFinished(int)) );
...
{
	QVariantHash serviceProviderData = m_uiStop.serviceProvider->model()->index( index, 0 )
			.data( ServiceProviderDataRole ).toHash();
