...
void GraphicalTimetableLine::createTooltip( Departure *departure )
{
	...
	if ( m_departures.isEmpty() ) {
		...
		if ( dataList.count() == 1 ) {
			...
			// Mulitple departure item hovered
			QString text;
...
// 	text.append( minTime == maxTime ? KGlobal::locale()->formatTime(minTime.time())
// 			: QString("%1 - %2").arg(KGlobal::locale()->formatTime(minTime.time()))
// 								.arg(KGlobal::locale()->formatTime(maxTime.time())) );
...
void GraphicalTimetableLine::resizeEvent(QGraphicsSceneResizeEvent* event)
{
	...
	qreal scale = qMax( 0.4, qMin(1.0, qMin(rect.width(), rect.height()) / 250) );
	foreach ( Departure *departure, m_departures ) {
