...
void PublicTransportRunner::reloadConfiguration()
{
    ...
    syntaxDepartures.setSearchTermDescription(i18nc("A description of the search term for the 'departures' keyword", "stop"));
    
    Plasma::RunnerSyntax syntaxArrivals(m_settings.keywordArrival + QLatin1String(" :q:"),
	       i18n("Finds public transport arrivals at :q:."));
    syntaxArrivals.setSearchTermDescription(i18nc("A description of the search term for the 'arrivals' keyword", "stop"));
    
    Plasma::RunnerSyntax syntaxJourneys(m_settings.keywordJourney + QLatin1String(" :q:"),
	       i18n("Finds public transport journeys :q:."));
    syntaxJourneys.setSearchTermDescription(i18nc("A description of the search term for the 'journeys' keyword", "origin stop to target stop"));
    
    Plasma::RunnerSyntax syntaxStops(m_settings.keywordStop + QLatin1String(" :q:"),
	       i18n("Finds public transport stops like :q:."));
    syntaxStops.setSearchTermDescription(i18nc("A description of the search term for the 'stops' keyword", "stop part"));
		    ...
		    Plasma::DataEngine *engine, Plasma::RunnerContext* c)
{
    ...
    keywordMap.insert( i18nc("This is a runner keyword to search for bus/tram/interurban train/metro/trolley bus departures",
			     "public transport from"),
	PublicTransportRunner::OnlyPublicTransport | PublicTransportRunner::Departures );
    keywordMap.insert( i18nc("This is a runner keyword to search for bus/tram/interurban train/metro/trolley bus arrivals",
			     "public transport to"),
    ...
    QRegExp rx(i18nc("This is a regular expression, used for the runner query string, to give the offset in minutes of the first result. '\\b' at the beginning and at the end assures, that the found string is seperated from other words with whitespaces. The '(\\d+)' is used to match an integer and there must be one such part in the regexp (there shouldn't be parenthesized expressions before this one, but you can use non-matching parentheses '(?:XX)'). 'min(?:utes)?' can match 'min' and 'minutes'. The string is matched case insensitive.",
...
// 	KGlobal::locale()->formatDateTime(departure, KLocale::FancyShortDate)
	...
	QString delayText = delay == 0
	    ? ", " + i18nc("Used to indicate that a train, bus, etc. is departing/arriving on time", "on schedule")
...
// 	QDateTime predictedArrival = delay > 0 ? arrival.addSecs(delay * 60) : arrival;
	...
	int minsToDeparture = QDateTime::currentDateTime().secsTo(predictedDeparture) / 60;
	QString durationDep = minsToDeparture == 0 ? "" : KGlobal::locale()->formatDuration(minsToDeparture * 60 * 1000);
	int minsToArrival = QDateTime::currentDateTime().secsTo(predictedArrival) / 60;
	QString durationArr = minsToArrival == 0 ? "" : KGlobal::locale()->formatDuration(minsToArrival * 60 * 1000);
	...
	if ( journeyDuration > 0 ) {
	    subtexts << i18nc("The duration of a journey",
			      "Duration: %1", KGlobal::locale()->formatDuration(journeyDuration * 60 * 1000));
	}
	if ( changes >= 0 ) {
